...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/v1/snapshot**", async (route) => {
    ...
    snapshot.services = snapshot.services.map((service: Record<string, any>) => service.id === "coffee-machine"
      ? { ...service, actions: service.actions.map((action: Record<string, any>) => ({ ...action, enabled: true })) }
      : service);
