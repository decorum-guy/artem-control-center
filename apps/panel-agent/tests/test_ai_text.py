...
def test_store_never_returns_plaintext_and_keeps_other_credentials(tmp_path):
    ...
    store.select(expected_revision=2, provider_id="deepseek", model="deepseek-chat")
...
def test_gigachat_exchanges_and_caches_token_then_normalizes_response(tmp_path):
    ...
    service = AITextService(settings(tmp_path), store, transport=httpx.MockTransport(handler))
...
def test_tier1_bypasses_transport_and_cloud_failure_uses_local_fallback(tmp_path):
    ...
    service = AITextService(settings(tmp_path, ai_local_enabled=True), store, transport=httpx.MockTransport(handler))
    ...
    assert result.provider_id == "local" and result.fallback_used is True and result.text == "Локальный ответ."
...
def test_yandex_and_deepseek_translate_through_fixed_adapters(tmp_path):
    ...
    store.select(expected_revision=3, provider_id="deepseek", model="deepseek-chat")
...
def test_malformed_provider_response_is_sanitized(tmp_path):
    ...
    store.select(expected_revision=1, provider_id="deepseek", model="deepseek-chat")
