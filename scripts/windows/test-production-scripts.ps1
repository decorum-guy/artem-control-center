...
try {
    ...
    $kioskPresenceText = Get-Content `
        ...
        -Raw
    $configureHomeText = Get-Content `
        -LiteralPath (Join-Path $PSScriptRoot "configure-home-production.ps1") `
        -Raw
    ...
    foreach ($gate in @(
        ...
        if ($configureHomeText -notmatch [regex]::Escape($gate)) {
            throw "Home production configurator must keep the gate disabled during verification: $gate"
        }
    }
    foreach ($gate in @(
        ...
        if ($configureHomeText -notmatch [regex]::Escape($gate)) {
            throw "Home production configurator is missing the approved final gate: $gate"
        }
    }
    if ($configureHomeText -notmatch '\$KeepWritesDisabled') {
        throw "Home production configurator must retain the explicit writes-disabled safety switch"
    }
