...
function Stop-ArtemTargetContinuationForRecovery {
    ...
    try {
        # Use the exact Process object returned by Start-Process so recovery
        # cannot target an unrelated recycled PID.
        $TargetProcess.Kill()
    }
    catch {
        $TargetProcess.Refresh()
...
function Wait-ArtemTargetContinuationAcceptance {
    ...
    return $false
        ...
        [Parameter(Mandatory)][ValidatePattern('^[0-9a-f]{40})][string]$LockRequestId
    )
    $evidencePath = Join-Path $Paths.Logs ("update-handoff-{0}.json" -f $LockRequestId)
    $payload = Get-ArtemJsonPayload -Path $evidencePath
    if (
...
function Wait-ArtemTargetContinuationAcceptance {
    ...
    $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
    while ((Get-Date) -lt $deadline) {
        # Bootstrap acceptance is the responsibility-transfer boundary. A
        # lease claim alone is intentionally insufficient: the physical #240
        # failure occurred after claim but before this durable marker.
        ...
        # Keep validating that the exact child remains the handoff owner while
        # waiting. A missing claim is normal briefly after launch; a different
        # owner can never become successful without the exact bootstrap marker.
        $lock = Get-ArtemJsonPayload -Path $Paths.UpdateLock
        if (
            $null -ne $lock -and
            (Test-ArtemTargetHandoffLease -Existing $lock -LockRequestId $LockRequestId -Current $Current -Target $Target) -and
