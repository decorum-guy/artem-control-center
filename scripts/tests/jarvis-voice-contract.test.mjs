...
test("Windows voice task is interactive, limited, optional, and rejects bad sessions", () => {
  ...
  assert.match(start, /PANEL_JARVIS_VOICE_ENABLED=true/);
  assert.match(start, /Refusing to start duplicate Jarvis voice workers/);
  assert.match(status, /\$_ -eq 0 -or \$_ -ne \$consoleSession/);
  assert.match(runtime, /jarvis-voice/);
  for (const source of [install, start, status, runtime]) {
